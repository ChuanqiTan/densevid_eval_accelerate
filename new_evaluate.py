...
class new_ANETcaptions(object):
    ...
    def evaluate_tiou(self, tiou):
        def tokenize_by_single(res, gts):
            link = {}
            global_id = 0

            res2 = {}
            gts2 = {}
            for vid in gt_vid_ids:
                if res[vid] and gts[vid]:
                    for i in res[vid]:
                        res2[global_id] = res[vid][i]
                        gts2[global_id] = gts[vid][i]
                        link[global_id] = (vid, i)
                        global_id += 1

            res2_token = self.tokenizer.tokenize(res2)
            gts2_token = self.tokenizer.tokenize(gts2)
            ...
            for gi in link:
                ...
                res_t[vid][i] = res2_token[gi]
                gts_t[vid][i] = gts2_token[gi]

            return res_t, gts_t


        gt_vid_ids = self.get_gt_vid_ids()
        res, gts = self.eval_step1_pair(tiou)
        res_t, gts_t = tokenize_by_single(res, gts)
        ...
        for scorer, method in self.scorers:
            ...
            for vid_id in gt_vid_ids:
                ...
                if len(res[vid_id]) == 0 or len(gts[vid_id]) == 0:
                    ...
                    score, scores = scorer.compute_score(gts_t[vid_id], res_t[vid_id])
